    ...
    public static class DeviceInfo
    {
        ...
        public static SoftwareArchitecture ProgramBits
        {
            get
            {
                ...
                {
                    case 64:
                        pbits = SoftwareArchitecture.x64;
                        break;
                ...
                try
                {
                    ...
                    switch (l_System_Info.uProcessorInfo.wProcessorArchitecture)
                    {
                        case 9: // PROCESSOR_ARCHITECTURE_AMD64
                            pbits = ProcessorArchitecture.x64;
                            break;
        ...
        #region CPU
        /// <summary>
        /// Get CPUInfo [ Cores, Manufacture & ClockSpeed ]
        /// </summary>
        /// <returns></returns>
        public static List<string> CPU()
        {
            ...
            try
            {
                foreach (var obj in new ManagementObjectSearcher("select NumberOfCores, Manufacturer, MaxClockSpeed from Win32_Processor").Get())
                {
                    cpuInfo.Add(obj["Manufacturer"].ToString());
                    cpuInfo.Add(obj["NumberOfCores"].ToString());
                    var clockSpeed = decimal.Round((decimal)0.001d * (uint)obj["MaxClockSpeed"], 2, MidpointRounding.AwayFromZero);
                    cpuInfo.Add(string.Format("{0} GHz", clockSpeed));
                }
            }
            catch
            { }
        ...
        {
            foreach (var obj in new ManagementObjectSearcher("select TotalPhysicalMemory FROM Win32_ComputerSystem").Get())
            {
                return Bytes.GetSize((UInt64)obj["TotalPhysicalMemory"]);
            }
